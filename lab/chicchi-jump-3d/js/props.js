...
const ASTEROID_CHUNKS = [
  ...
  [-0.06, -0.02,  0.00, 0.375, 0.62, [ 0.45,  0.9,  0.30]],   // 主役
  [ 0.36,  0.05, -0.04, 0.255, 0.70, [ 1.10, -0.5, -0.75]],
  [-0.40, -0.05,  0.05, 0.230, 0.66, [-0.60,  1.7,  0.95]],
  [ 0.16,  0.14,  0.10, 0.165, 0.72, [ 1.90,  0.3, -0.40]],
  [-0.20,  0.13, -0.08, 0.140, 0.68, [ 0.25, -1.2,  1.40]],
  [ 0.60, -0.06,  0.02, 0.120, 0.60, [-1.30,  0.7,  0.20]],
  [-0.62, -0.04, -0.03, 0.105, 0.64, [ 0.80,  2.2, -0.90]],
  [ 0.03, -0.16,  0.14, 0.135, 0.58, [ 1.55, -1.8,  0.55]],
];
...
function asteroidChunks(b, colFn) {
  const s = sphereGeometry(7, 5);     // 分割を粗くして、面をはっきり切る
...
function buildRockMesh() {
  ...
  return asteroidChunks(new PropBuilder(), (px, py) => {
...
function buildRockIceMesh() {
  const b = new PropBuilder();
  const s = sphereGeometry(6, 4);      // さらに粗く＝面が大きく、結晶らしくなる
  for (const [x, y, z, r, sq, rot] of ASTEROID_CHUNKS) {
    b.add(s, {
  ...
  // 上へ突き出るとがった氷。これが「氷だ」と分かる決め手。
  const spike = loftGeometry({
    rings: 4, radial: 5, k: 2.0, minR: 0.006,
    profile: t => { const r = 0.075 * (1 - t * 0.94); return [r, r]; },
    spine: t => [0, t * 0.26, 0],
  });
  for (const [x, z, sc, rz] of [[-0.10, 0.05, 1.0, 0.18], [0.17, -0.05, 0.72, -0.30], [-0.02, 0.12, 0.55, 0.42]]) {
    b.add(spike, {
      offset: [x, 0.10, z], scale: [sc, sc, sc], rot: [0, 0, rz],
      center: [x, 0.10, z], flat: true, col: [1.15, 1.25, 1.45],
    });
  }
  return b.build();
}
...
// ---------------- 💥 くだける小惑星（宇宙ゾーンのこわれ足場）----------------
// かたまりのあいだを大きくあけて、いまにもバラバラになりそうに見せる。
