...
(function () {
  ...
  function applySettings(kind, entry) {
    ...
    // ★灘中対策の問題の紙の厚みは 1 で固定する（設定のスライダーは反映しない。
    //   本人指示2026-09-05「紙の厚みを1にして、回転問題は回転部を少し浮かしたら？」）。
    //   ・厚み0だと折り返した紙と土台が**完全に同一平面**になり、重なった部分で
    //     土台（表の色）が描画で勝つ＝裏返した紙が表の色に見えた
    //     （本人「裏返った紙の色が表の紙の色と一緒になる」）。
    //   ・厚みを入れると flatStackStep が効いて、重なった層が紙の厚みぶん
    //     上へ積まれる＝実物の紙と同じ重なり方になり、裏の色がちゃんと出る。
    //     回転（重ね合わせ）問題で回す紙が浮くのも同じ仕組み。
    //   ・以前ここを0にした理由（2026-09-03「重ね合わせの問題がバグる」）は、
    //     折り目の巻きこみ帯の判定が縦軸で退化して紙が多角形にふくらむ不具合で、
    //     その原因（axH < 1e-6）は thickenMesh 側で直してある。
    //     再発の見張りは check_problems.py の「帯の本数」の検査が受け持つ。
    //   ・設定のスライダーを反映しないのは本人指示（厚みは反映しなくてよい）。
    const isProblem = (kind || currentKind) === 'problem';
    inst.setThickness(isProblem ? 1 : settings.thick);
  }
