# -*- coding: utf-8 -*-
"""小4灘合 第3回に足した5本（大問1・2・4・5・7）を、実際のアプリ画面で目視＋打ちこみ確認する。
...
使い方: python scripts/g4n03_shot.py
"""
...
OUT = os.path.join(BASE, "scripts", "_g4n03_shots")
PORT = 8763
WANT = [("hd4n_03_6", "四分円", ["15.7"]),
        ("hd4n_03_7", "ぬりつぶした角", ["540"]),
        ("hd4n_03_8", "正方形の中の四角形", ["75"]),
        ("hd4n_03_9", "㋐と㋑", ["1/12", "1/9"]),
        ("hd4n_03_10", "45", ["34.5"])]
...
def main():
    ...
    with sync_playwright() as p:
        ...
        def open_picker(first=False):
            ...
            pg.wait_for_timeout(1600)
            pg.evaluate("(async()=>{ setHamaCurrent(4,'nadago',3); await renderHamaPanel(); })()")
        ...
        for rid, keyword, answers in WANT:
            ...
            for n, ans in enumerate(answers):
                ...
                if not type_answer(pg, ans, bad, rid):
                    break
                pg.wait_for_timeout(1100)
