...
def build():
    d1 = {
        "id": "hd3s_n14_1", "hg": "HG-1431", "src": "小3最レ【刷新版】No.14 大問1（実物・8月2回目の範囲）",
    ...
    d2 = {
        "id": "hd3s_n14_2", "hg": "HG-1432", "src": "小3最レ【刷新版】No.14 大問2（実物・8月2回目の範囲）",
    ...
    d3 = {
        "id": "hd3s_n14_3", "hg": "HG-1433", "src": "小3最レ【刷新版】No.14 大問3（実物・8月2回目の範囲）",
...
def genbo():
    ...
    meta_rest = [
        ("HG-1434", "大問4（しるしの角の和・形をつくりかえる）★★★",
         "外角定理で角をとなりへ寄せると、多角形の内角の和につくりかえられる",
         "とがりの数が同じなら、形がちがっても和は同じ（星型の一筆書き＝2回まわる＝180×(n-2×2)）",
         "あり（4問とも。実物は手がきで形が不正確なため、しるしの数と重なり方をそろえて引き直した）",
         "1つずつでは出ない角を和だけ出す発想の初出。(1)(3)は同じ5つのとがりで"
         "形がちがっても答えは同じという一段深い気づきが仕込んである。(2)(4)は三角形/四角形が"
         "重なった形の内角の和×2で出す。★3＝小4でもう一度出る単元。今回は定着でなく初めて触れることが目的"),
        ("HG-1435", "大問5（しるしの角の和・もっと大きな形／逆算）★★★",
         "重なっている多角形を見わけて、それぞれの内角の和をたす",
         "(3)は逆に発想する：8つの角の和が360度と決まっているので、7つを引けばxが出る",
         "あり（3問とも）",
         "(1)(2)は大問4の延長（三角形+四角形＝540、六角形+三角形＝900）。"
         "(3)は骨がちがう：1つずつ出そうとせず、和が決まっていることに気づけばひき算1回。"
         "座標で8つの角を1つずつ指定して閉じる多角形を作り、実際に足して360になることを検算した"),
        ("HG-1436", "大問6（同じしるしの角・2つ分3つ分で持つ）★★★",
         "○や●の1つ分が分からないまま、何つ分かだけで計算をすすめる",
         "○＋●の和（例:125度）を先に出し、それを2つ分・3つ分に掛けてから、"
         "四角形や三角形の内角の和・外角定理と組みあわせる",
         "あり（4問とも）",
         "(1)(2)は○+●の和を出してから2倍、(3)(4)は3等分の角+外角定理を組みあわせる、と"
         "難度が段階的に上がる。1つ分の大きさは最後まで分からなくてよい——量を保ったまま式変形する"
         "この回いちばんの発見"),
        ("HG-1437", "大問7（同じしるしの角・まとめ）★★★",
         "式をまるごと2倍する／たし算で3段の関係をつなぐ",
         "(1)は小さい三角形の式(○+22=●)をまるごと2倍して大きい三角形の式と比べる。"
         "(2)は3重の三角形の内角の和を2つ足し合わせて重複を消す",
         "あり（2問とも）",
         "この回のしあげ。(1)は式をまるごと2倍するという抽象操作が初出（小3にはかなり重い）。"
         "(2)は3つの式をたす・とく発想で、覆面算的な連立の感覚に触れる。★3の最後として、"
         "定着ではなく「こういう考え方がある」に触れさせるのが目的（本人確認：4年生でまた出る）"),
    ]

    def emit(r, hg, title, hone, core, zu, memo):
        out.append(u"### 【%s】小3最レ（刷新版）No.14 %s" % (hg, title) + chr(10))
        ...
        out.append(u"- 図: %s ／ 答え: %s" % (zu, ans.replace("(4) 6度", "(4) 六角形（□＝6）")) + chr(10))
        for i, st in enumerate(r["steps"]):
            out.append(u"- 解法(%d): %s" % (i + 1, st["meaning"].replace("**", "")) + chr(10))
        out.append(u"- 作問メモ: %s" % memo + chr(10))
        out.append(u"- アプリ実装: `data/hama_daimon.json` grades.3.sairei_new_bunsatsu.fukushu[" + chr(34) + "14" + chr(34) + "] の `%s`" % r["id"] +
                   u"（生成元 `scripts/gen_s3sairei_no14.py`。**JSONを手で書かない**）" + chr(10))
        for i, st in enumerate(r["steps"]):
            if st.get("svg"):
                out.append(u"- 図SVG(%d):" % (i + 1) + chr(10) + "```html" + chr(10) + st["svg"] + chr(10) + "```" + chr(10))
        out.append(chr(10))

    for r, (hg, title, hone, core, zu, memo) in zip(recs, meta):
        emit(r, hg, title, hone, core, zu, memo)
    for r, (hg, title, hone, core, zu, memo) in zip(build_rest(), meta_rest):
        emit(r, hg, title, hone, core, zu, memo)
    return "".join(out)
...
def fig_6_3():
    ...
    # ★xの弧は「Eから見てDの方向（Tと反対＝下向き）」と「Eから見てLと反対の方向（上向き）」の
    #   対頂角がわに描く。実物と同じ側（監査の指摘・2026-09-06）。数値は対頂角なので同じ129度。
    farT = polar(E, ang_of(T, E), 60)   # Eから見て T と反対（＝D1のさらに先）
    farL = polar(E, ang_of(L, E), 60)   # Eから見て L と反対
    s = Svg(380, 268)
    s.poly([T, L, R]); s.line(T, D1); s.line(T, D2); s.line(L, polar(L, 18, 300))
    s.text(s.arc(D1, L, T, 30), "111°")
    s.text(s.arc(R, T, L, 28), "45°")
    s.text(s.arc(E, farT, farL, 26, COL_ARC2), "x", COL_ASK, 17, italic=True)
    ...
    must("6(3) 答えのx", angle_at(E, farT, farL), 129)
    return s.out()
...
def fig_5_1():
    # ★2026-09-06：塾講師監査の指摘で描き直した。
    #   もとは三角形と四角形を「中心が同じ・別々の輪」として重ねていたので、
    #   頂点が7か所（3+4）に分かれて見えた。実物は**2つの頂点を共有**していて、
    #   しるしが実際に見えるのは5か所だけ（共有点2つ・単独3つ）。
    #   ★共有点では、三角形の角と四角形の角の**2つの弧をすぐ隣に描く**
    #   （角の大きさそのものを合成しない＝計算をまちがえない一番かたい書き方）。
    #   合計は 三角形180度＋四角形360度＝540度 のまま変わらない。
    P1 = P(58, 44)                          # 三角形だけの頂点
    S1 = P(276, 92)                         # 共有頂点①（三角形・四角形の両方）
    S2 = P(138, 278)                        # 共有頂点②（三角形・四角形の両方）
    Q1 = P(328, 214)                        # 四角形だけの頂点
    Q2 = P(38, 158)                         # 四角形だけの頂点
    tri = [P1, S1, S2]
    quad = [S1, Q1, S2, Q2]
    s = Svg(366, 316)
    ...
    for T in (tri, quad):
        n = len(T)
        for i in range(n):
            s.arc(T[i], T[(i - 1) % n], T[(i + 1) % n], 22 if T is tri else 34)
...
def build_rest():
    """大問4〜7（11問）"""
    d4 = {
        "id": "hd3s_n14_4", "hg": "HG-1434", "src": "小3最レ【刷新版】No.14 大問4（実物・8月2回目の範囲）",
        ...
        "steps": [
            {"question": "しるしが ついている角の 大きさの 和は 何度ですか。", "answer": "180", "svg": fig_4_1(),
             "meaning": "①星の とがりは 5つ。1つずつは 分かりません。"
                        "②となりあう 2つの とがりに 注目します。その辺を のばすと、"
                        "**2つの角を たした大きさが、のばした線の 交わった点に できる角と 同じ** になります"
                        "（外角の性質）。"
                        "③そうやって 角を 1か所に 寄せていくと、のこりの3つと あわせて **1つの三角形**に なります。"
    ...
    d5 = {
        "id": "hd3s_n14_5", "hg": "HG-1435", "src": "小3最レ【刷新版】No.14 大問5（実物・8月2回目の範囲）",
    ...
    d6 = {
        "id": "hd3s_n14_6", "hg": "HG-1436", "src": "小3最レ【刷新版】No.14 大問6（実物・8月2回目の範囲）",
    ...
    d7 = {
        "id": "hd3s_n14_7", "hg": "HG-1437", "src": "小3最レ【刷新版】No.14 大問7（実物・8月2回目の範囲）",
        ...
        "steps": [
            {"question": "角xの 大きさは 何度ですか。", "answer": "44", "svg": fig_7_1(),
             "meaning": "①まず 小さい三角形で 外角定理：○＋22度＝**●**。"
                        ...
                        "③②の式には ○○（○2つ分）と ●●（●2つ分）が 出てきました。"
                        "①の式を **そのまま2倍** すれば、②と 同じ ○○・●● の形に そろえられます。"
                        "○2つ分＋44度＝●2つ分。"
                        "④②と くらべると、x＝22×2＝**44度**。"
                        "⑤**式をまるごと2倍して 形をそろえる**——これが この問題の 手です。"},
            {"question": "角イと 角ウの 大きさの 和が 240度の とき、角アの 大きさは 何度ですか。", "answer": "60", "svg": fig_7_2(),
