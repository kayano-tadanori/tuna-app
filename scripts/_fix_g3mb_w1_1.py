# -*- coding: utf-8 -*-
"""docs/_audit/g3mb_w1/findings_1.md の指摘を hama_daimon.json に当てるパッチ。

  python scripts/_fix_g3mb_w1_1.py [対象JSON]      （省略時 data/hama_daimon.json）

対象: 小3マスター算数第1分冊 No.1〜No.4（hd3mb_01_1〜hd3mb_04_14・HG-4001〜HG-4054）。
54本を原簿と突き合わせた結果、軽微な指摘1件のみ（findings_1.md参照）。
...
EDITS = [
    # ---------------------------------------------------------------- 軽1
    # hd3mb_01_17（HG-4017）小問1の解説が「⑥を⑨として使う」核心の手順を
    # 説明せずに答え95410へ飛んでいる。introには裏返しルールが書いてあるが、
    # 解説（meaning）はそれを一度も参照しない。小問1の解説にひとこと足す。
    ("hd3mb_01_17", [
        ("R", "steps/0/meaning",
         "大きい順に並べると95410になる。",
         "⑥のカードを⑨として使うと数字は0,1,4,5,9になる。大きい順に並べると95410になる。"),
    ]),
]
