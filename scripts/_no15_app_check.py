# -*- coding: utf-8 -*-
u"""小3最レ（刷新版）No.15 の全7大問19問を、実際のアプリ画面で 打ちこんで 〇が出るまで見る。
...
使い方: python scripts/_no15_app_check.py
"""
...
# 大問ごとの 答え（解答ページ p42 のとおり）
ANSWERS = [
    (u"正方形と正三角形", ["75", "60"]),
    (u"円の半径", ["98", "45"]),
    (u"おうぎ形", ["45", "52", "68"]),
    (u"かくれた二等辺", ["65", "68", "56"]),
    (u"合同な三角形を さがす", [u"キ", u"ク", u"カ", u"オ"]),
    (u"合同だから", ["117", "40", "120"]),
    (u"いどうさせる", ["70", "30"]),
]
...
def main():
    ...
    with sync_playwright() as p:
        ...
        def open_panel():
            ...
            # ★コースは sansuState.hamaCourse で決まる。setHamaCurrent だけでは切りかわらない
        ...
        for di, (keyword, answers) in enumerate(ANSWERS):
            ...
            if idx < 0:
                ...
                continue
            if di > 0:
                open_panel()          # ★クイズ画面から じゅくナビへ戻ってから 押す
                pg.click('.hama-act-btn[data-hama-act="%s"]' % cand[0]["key"])
