...
実物で確かめたページ対応：
  ...
  HG-0683 … No.6 V(2枚目)大問8。1〜35の数表＋わく2種（⑦=たて2よこ3・和33の例、
             ④=3×3まん中なし・和216の例）
  HG-0733 … No.18 V(2枚目)大問7。3つの歯車A・B・Cが横一列に外接
             （⚠「歯数比3:4:7」は問題文に書いてあるだけで**実物の図には印字されていない**）
...
# ★viewBox は 2026-09-06 の監査（_fix_g5m_w2_2.py）が実データ側で広げたもの。
#   ここを "0 0 300 160" に戻すと、流し直すたびアプリと食いちがう。
FIGS["HG-0733"] = svg("-20.6 0 341.2 160", "".join(_gear + [
    # ★2026-09-09：キャプションを外した。原本（_結合\小5\小5_算数_復習テスト_実力〜No.24.pdf
    #   p37＝No.18 3枚のうちの2枚目 大問7）を600dpiで拡大して確かめたところ、
    #   図に印字されているのは「A」「B」「C」の3文字だけだった。
    #   とくに「（歯数の比A:B:C＝3:4:7）」は**これから問う答えの先出し**で、原本にも無い。
    #   前半「歯車A・Bがかみあい…」も原本の図には無いが、問題文が言っている内容の言いかえで
    #   答えを漏らさないため、アプリ・原簿に入っているものに合わせて残す。
    t(150, 150, "歯車A・Bがかみあい、B・Cもかみあう", GRAY, 11),
]))
