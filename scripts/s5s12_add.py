...
# ── 大問の題名（「大問をえらぶ」画面に子どもが見る文字）を掃除する ──
# 原簿の題名は制作側のメモなので「HG-6619の発展」「同じ骨」「衣装違い」「要実物再照合」が入っている。
# そのまま出すと子どもの画面に内輪の言葉が出る（2026-09-02に65本が出ていた）。
_HGREF = r"HG-\d+(?:\s*[〜~／/・]\s*(?:HG-)?\d+)*"
# 子どもが読まない言葉・小5が習っていない道具は言いかえる
_TITLE_WORD = [
    ("分数の部分分数分解・télescoping和", "分数を差に分けて次々に打ち消し合う和"),
    ("分数の連続積・télescoping積", "分数のかけ算が次々に打ち消し合う積"),
    ("binary→decimal写像", "2進法から十進法へのうつしかえ"),
    ("2辺と挟角のsinθ公式", "同じ角をはさむ2辺の積で面積をくらべる"),
    ("セビアン", "頂点から向かい合う辺へ引いた線"),
    ("・keisan型", ""), ("，keisan型", ""), ("、keisan型", ""), ("keisan型", ""),
    ("CRT", "あまりの条件が3つ"), ("LCM", "最小公倍数"),
]
_TITLE_FIX = {          # 機械の掃除だけでは文がおかしくなるものは手で決める
    "HG-6641": "ヒント無し版＋分母が奇数間隔",
    "HG-6708": "8進法のメーター・素直な8進法",
    "HG-7102": "のりしろ・すきまをはさんで正方形を作る（最小公倍数）",
    "HG-7103": "あまりがちがう3つの条件から数を求める仕上げ",
    "HG-7125": "直角三角形2枚を重ねた蝶形の相似・基本形と複合図形の2問",
    "HG-7161": "正三角形7個でできた五角形・平行線を使った長さの発見",
    "HG-7064": "棒のかげが段だんの上にとどく・4問",
    "HG-7172": "すれちがい算・長さと速さを逆算する2問",
    "HG-7125": "直角三角形2枚を重ねた蝶形の相似",
    "HG-6692": "記号の約束・1からその数までかけた数のわり算",
    "HG-7092": "平行線でつながる相似＋交わる点で分かれる比の複合2問",
    "HG-7104": "1からその数までかけた数・素因数の個数と末尾の0の個数",
    "HG-7111": "1からその数までかけた数にふくまれる素因数の個数",
    "HG-6868": "頂点から向かい合う辺へ引いた線の上の点が作る四角形と、もとの三角形の面積比",
    "HG-7081": "わる数もあまりもちがう2つの条件・基本形2問",
    "HG-7082": "「たすとわりきれる」「ひくとわりきれる」型・2問",
    "HG-7083": "あまりの条件が3つ＋1000にいちばん近い数",
    "HG-7122": "往復旅人算・「n回目に出会う」から道のりを逆算する",
    "HG-7059": "直角三角形の斜辺への垂線＝相似の基本の比・3問",
    "HG-6577": "同じ□が入る式・線分図で解く",
    "HG-6694": "記号の約束・「aをb回かけるとcになる」の逆算",
    "HG-6800": "相当算・「ある数」をもとめる",
    "HG-7117": "3種類のつるかめ算・同じ本数の2種をまとめて考える",
    "HG-7036": "100〜200の中の倍数の個数・どちらでもわりきれない数",
    "HG-7020": "2つの正方形をならべた図／平行四辺形を辺の外までのばした図・2問",
    "HG-6641": "分数を差に分けて次々に打ち消し合う和・2問",
    "HG-6897": "台形の対角線2本が作る4つの部分の面積比・3問",
    "HG-7127": "2つの直角三角形を重ねた交点の高さ",
    "HG-7179": "長方形の中で垂直に交わる2本の直線",
    "HG-7078": "平行線でできる相似・DE：BCの比から辺の長さを求める",
    "HG-6907": "垂線の足が作る面積比／正方形の中の交点四角形の面積比・2問",
    "HG-6649": "1から999までで2つの数字が両方使われている個数",
...
def tidy_title(t, hg=None):
    if hg in _TITLE_FIX:
        return _TITLE_FIX[hg]
    s = t
    s = re.sub(r"[／/・＋+]?\s*（?\(?\d?\)?は?要実物再?照合の", "", s)
    s = re.sub(r"[／/・＋+]?\s*(?:（\d\)|\(\d\)は|)\s*要実物再?照合", "", s)
    s = re.sub(r"[＝=]\s*%s\s*(?:と同系統だが今回は|の言い換え|の?[^・]*)" % _HGREF, "", s)
    s = re.sub(r"[・／/]\s*%s\s*の[^・／/]*" % _HGREF, "", s)
    s = re.sub(r"^%s\s*(?:系)?(?:と同じ骨)?の?" % _HGREF, "", s)
    s = re.sub(r"%s\s*(?:系)?(?:と同じ骨)?の?" % _HGREF, "", s)
    s = s.replace("同じ骨", "同じ考え方").replace("骨", "考え方")
    s = s.replace("衣装違い", "ちがう場面").replace("衣装", "場面")
    for x, y in _TITLE_WORD:
        s = s.replace(x, y)
    s = re.sub(r"[・／/]{2,}", "・", s)
...
def _put_svg(r, key, steps, n0):
    """小問ごとの図（原簿の「- 図SVG（(1)）:」）を、その小問から作ったstepに付ける。
       アプリ側は step.svg || 大問の svg の順に見る（js/sansu.js）。
       大問ぜんぶで1枚の図（key=0）は大問側に付けるので、ここでは付けない。"""
    sv = r["svgs"].get(key)
    if sv and key != 0:
        for st in steps[n0:]:
            st["svg"] = sv
