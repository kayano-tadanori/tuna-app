# -*- coding: utf-8 -*-
"""docs/_audit/g3mb_w4/findings_1.md の指摘を hama_daimon.json に当てるパッチ。

  python scripts/_fix_g3mb_w4_1.py [対象JSON]      （省略時 data/hama_daimon.json）

対象: 小3マスター算数第2分冊 復習No.21〜No.22（hd3mb_21_7〜hd3mb_22_3・HG-4228〜HG-4240）。
11本を原簿と突き合わせ、独立検算・SVG座標の幾何検算を行った結果、
重大3件（hd3mb_22_1 / hd3mb_22_2 / hd3mb_22_3）・中1件（hd3mb_21_7）・
軽1件（hd3mb_21_14）を見つけたが、**EDITSは空**（詳細はfindings_1.md）。

  ★重大3件（展開図②④/①③⑥/頂点ア〜セ）は、SVGの升目座標をロールアルゴリズムで
    検算した結果「答えとされた展開図が実際には折りたためない」「6面必要な展開図が
    5面しかない」ことまでは証明できたが、正しい図を描き直すには原本PDF
    （`浜問題\3年算数\3年マスター算数_第2分冊.pdf` 本文p77-78）の実物確認が必須。
    今回の作業環境はG:ドライブ（Google Drive、原本PDFの置き場）が未マウントで
    原本に一切アクセスできなかった。想像で図を描き直すのは「原簿通りにやる」
    ルール違反になるため、SVGの修正は書いていない（→[[feedback_genbo_dori]]）。
  ★中1件（hd3mb_21_7）は原簁の6設問中3問が未実装というカバレッジの問題で、
    既存内容の誤りではなく新規小問の追加（作問）に相当するため見送り。
  ★軽1件（hd3mb_21_14）はintroの追加説明文が内容として誤りではないため見送り。
...
決めごと（過去の事故から）
  ...
  * 図SVGを直す修正は、入れる前に座標を検算し、合わなければ1件も書かずに止める
    → 今回はその検算自体が「原本と食いちがう」という結論になったため、そもそも
      EDITSに載せていない（載せて止めるのではなく、最初から載せない）
"""
import io
import json
import os
...
# 今回は監査の結果、安全に適用できる修正が無かったため空（findings_1.md参照）。
EDITS = [
]
...
def main(argv):
    ...
    if not EDITS:
        log("EDITSが空（今回の監査では安全に適用できる修正が無かった。findings_1.md参照）。ファイルは書きかえない。")
        return 0
