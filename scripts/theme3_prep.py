...
def dump_pages(pdf, tag, pages, outdir):
    ...
    for pg in sorted(set(pages)):
        # ★練習問題の無いテーマ（No.18 テーマ2＝解説2ページだけ）はページ番号が0で来る。
        #   d[0 - 1] は**PDFの最後のページ**になり、無関係な画像を取り出していた（2026-09-18）
        if pg < 1:
            continue
...
- **テーマ%d**（しおりで実測）。1テーマ＝解説1p＋練習問題1p、解答は練習問題1つにつき1p。
  ...
  ⚠**「練習問題」の見出しは分冊でちがう**（第3分冊＝印刷されていない／**第1分冊＝「練習問題N」と番号つきで印刷**／
  **第2分冊＝「練習問題」とだけ印刷され番号は無い**。2026-09-18に実測）。**印刷されているものだけ写す。**
...
""" % (BOOK, no, title, os.path.basename(MONDAI), os.path.basename(KAITOU), n,
    ...
    if os.path.exists(prev):
        ...
        if not prev_title:
            # ★前の回が**別の分冊**のときは、M（いまの本のしおり）に無い。その本のしおりから取る。
            #   取れないと題名が前の回のまま残る
            #   （2026-09-18、No.11の指示書に第3分冊No.30の「相似(1)」が残った）
            prev_title = (toc_of(fitz.open(book_of(prev_no)[2])).get(prev_no) or {}).get('title', '')
        if prev_title:
            t = t.replace('No.%d %s' % (prev_no, prev_title), 'No.%d %s' % (no, title))
        # ★回番号の差しかえは「本の範囲（`No.11-20`）を食べない」ようにする。
        #   ただの replace だと `No.11-20` が `No.12-20` になり、担当に無い本の名前が渡る
        #   （2026-09-18、No.12で実際に起きた）
        t = re.sub(r'No\.%d(?![\d-])' % prev_no, 'No.%d' % no, t)
        ...
        # ★本の名前だけ直して回の範囲（`No.21-30`）が残ると、担当に別の本の名前を渡すことになる
        prev_tag = os.path.basename(book_of(prev_no)[2]).split('_')[-2]
        cur_tag = os.path.basename(MONDAI).split('_')[-2]
        if prev_tag != cur_tag:
            t = t.replace(prev_tag, cur_tag)
        # ★「原簿に何が入っているか」の一文は、分冊をまたぐと嘘になる（前の回の本の名前が残る）。
        #   回番号だけで言える形に毎回書き直す（2026-09-18、No.12で第2分冊＝No.21〜30 と書かれた）
        t = re.sub(r'原簿には[^\n]*\n(?:[^\n]*\n)??[^\n]*唯一の記録）。\n',
                   '原簿には同じ教材のほかの回が入っていますが、**この回（No.%d）は1本も入っていません**\n'
                   '（比べる相手がいない＝あなたの出力が唯一の記録）。\n' % no, t, count=1)
        # ★JSONの見本の `produced_by` に前の回の番号が残らないように
        t = re.sub(r'"produced_by": "G1担当A（No\.\d+）"',
                   '"produced_by": "G1担当A（No.%d）"' % no, t)
