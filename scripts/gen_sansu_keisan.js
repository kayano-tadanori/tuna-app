...
// 小4
// ============================================================
function g4_d1a() { // 同分母分数たし算（分数のまま答える。1を超える場合は帯分数）
...
function g4_d1b() { // 同分母分数の3口たし算
  const den = randInt(4, 10);
  const n1 = randInt(1, den - 1), n2 = randInt(1, den - 1), n3 = randInt(1, den - 1);
  const sum = new Frac(n1 + n2 + n3, den);
  const ansStr = sum.toAnswerString(true);
  if (ansStr === null) return null;
  return { question: `${n1}/${den} ＋ ${n2}/${den} ＋ ${n3}/${den} ＝ □`, answer: ansStr, meaning: `(${n1}＋${n2}＋${n3})/${den}＝${sum}` };
}
function g4_d1() { return Math.random() < 0.5 ? g4_d1a() : g4_d1b(); }

function g4_d2a() { // 同分母分数ひき算（□に入る数）
...
function g4_d2b() { // 同分母分数たし算（□に入る数）
  const den = randInt(4, 12);
  const n1 = randInt(1, den - 2), n2 = randInt(1, den - 1 - n1);
  const sum = n1 + n2;
  return { question: `${n1}/${den} ＋ ${n2}/${den} ＝ □/${den}　□に入る数は？`, answer: String(sum), meaning: `${n1}＋${n2}＝${sum}` };
}
function g4_d2() { return Math.random() < 0.5 ? g4_d2a() : g4_d2b(); }

function g4_d3a() { // 型D簡易版：小数の逆算（1ステップ）
...
function g4_d4b() { // 帯分数÷□＝target
  const target = randInt(1, 6);
  const b1d = randInt(2, 6), b1n = randInt(1, b1d - 1);
  if (gcd(b1n, b1d) !== 1) return null;
  const whole = randInt(1, 4);
  const mixed1 = new Frac(whole * b1d + b1n, b1d);
  const X = mixed1.div(new Frac(target, 1));
  if (X.d > 30 || X.n > 100) return null;
  const ansStr = X.toAnswerString(true);
  if (ansStr === null) return null;
  if (ansStr.includes('.') && ansStr.split('.')[1].length > 2) return null;
...
function g5_d1b() { // 小数→分数変換（既約分数）
  const den = pick([2, 4, 5, 8, 10, 20, 25]);
  const num = randInt(1, den - 1);
  const f = new Frac(num, den);
  const dec = f.toDecimalString();
  if (dec === null) return null;
  return { question: `${dec} を分数で表すと？（約分できる形で）`, answer: f.toAnswerString(false), meaning: `${dec}＝${f}` };
}
function g5_d1() { return Math.random() < 0.5 ? g5_d1a() : g5_d1b(); }

function g5_d2a() { // 異分母分数たし算（□に入る数、通分先固定）
...
function g5_d2b() { // 異分母分数ひき算（□に入る数、通分先固定）
  const d1 = randInt(2, 6), d2 = randInt(2, 6);
  if (d1 === d2) return null;
  const lcm = d1 * d2 / gcd(d1, d2);
  const n1 = randInt(1, d1 - 1), n2 = randInt(1, d2 - 1);
  const num1 = n1 * (lcm / d1), num2 = n2 * (lcm / d2);
  if (num1 <= num2) return null;
  return { question: `${n1}/${d1} － ${n2}/${d2} ＝ □/${lcm}　□に入る数は？`, answer: String(num1 - num2), meaning: `通分して${num1}/${lcm}－${num2}/${lcm}＝${num1 - num2}/${lcm}` };
}
function g5_d2() { return Math.random() < 0.5 ? g5_d2a() : g5_d2b(); }

function g5_d3a() { // 型D：小数の逆算（浜学園④相当）
...
function g5_d3b() { // 小数逆算（ひき算バージョン）
  ...
  const rightPart = Number((diff / divisor).toFixed(4));
  const final = Number((left - rightPart).toFixed(4));
  if (!Number.isInteger(final * 100) || final <= 0) return null;
  return {
    question: `${a}×${b}－(${cn.toFixed(2)}－${dn.toFixed(2)})÷□＝${final}　□に入る数は？`,
    answer: String(divisor),
    meaning: `${a}×${b}＝${left}、(${cn.toFixed(2)}－${dn.toFixed(2)})＝${diff}、÷□＝${rightPart}`
...
function g6_d1b() { // 帯分数のわり算基礎
  const whole = randInt(1, 3), bn = randInt(1, 5), bd = randInt(2, 6);
  if (gcd(bn, bd) !== 1) return null;
  const A = new Frac(whole * bd + bn, bd);
  const cn = randInt(1, 6), cd = randInt(2, 6);
  if (gcd(cn, cd) !== 1) return null;
  const C = new Frac(cn, cd);
  const ans = A.div(C);
  const ansStr = ans.toAnswerString(true);
  if (ansStr === null) return null;
  if (/\.\d{3,}/.test(ansStr)) return null;
  return { question: `${whole}と${bn}/${bd} ÷ ${cn}/${cd} ＝ □`, answer: ansStr, meaning: `${A}÷${C}＝${ans}` };
}
...
function g6_d4b() { // 分数・小数混合の3項計算（波かっこなし、灘中レベル）
  const xn = randInt(2, 12), xd = randInt(2, 12);
  if (gcd(xn, xd) !== 1) return null;
  const x = new Frac(xn, xd);
  const decVal = randInt(10, 99) / 100;
  const dec = new Frac(Math.round(decVal * 100), 100);
  const sum1 = x.add(dec);
  const yn = randInt(2, 9), yd = randInt(2, 9);
  if (gcd(yn, yd) !== 1) return null;
  const y = new Frac(yn, yd);
  const target = sum1.mul(y);
  const targetStr = target.toAnswerString(true);
  if (targetStr === null) return null;
  if (targetStr.includes('.') && targetStr.split('.')[1].length > 2) return null;
  return { question: `(${xn}/${xd}＋${decVal.toFixed(2)})×${yn}/${yd} ＝ □`, answer: targetStr, meaning: `${x}+${decVal.toFixed(2)}=${sum1}, ×${y}=${target}` };
}
function g6_d4() { return Math.random() < 0.6 ? g6_d4a() : g6_d4b(); }
...
for (let grade = 1; grade <= 6; grade++) {
  for (let diff = 1; diff <= 4; diff++) {
    ...
    const target = grade >= 4 ? 60 : 20; // 小4以降は3倍に
    ...
    console.log(`grade${grade} diff${diff}: ${target}問生成OK`);
  }
}
