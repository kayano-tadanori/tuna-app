# -*- coding: utf-8 -*-
"""docs/_audit/g3mb_w5/findings_1.md の指摘を hama_daimon.json に当てるパッチ。

  python scripts/_fix_g3mb_w5_1.py [対象JSON]      （省略時 data/hama_daimon.json）

対象: 小3マスター算数第2分冊 No.26〜No.28（hd3mb_26_2〜hd3mb_28_12・HG-4297〜HG-4333）。
37本を原簿と突き合わせ、独立検算（周期算・日暦算・数の性質、約90項目）とSVG座標の
実測（10本）、原簿とアプリJSONの完全一致確認を行った結果、内容面の不具合は0件
（findings_1.md参照）。EDITSは空。
