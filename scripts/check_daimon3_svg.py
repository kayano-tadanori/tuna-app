# -*- coding: utf-8 -*-
"""第3分冊(kouza1/kouza2)のSVGを、method_svg_check の3種類で実測する。
...
使い方: python scripts/check_daimon3_svg.py
"""
...
def main():
    ...
    for kind in ("kouza1", "kouza2"):
        for no, sets in sorted(node.get(kind, {}).items(), key=lambda kv: int(kv[0])):
            for x in sets:
                if x.get("svg"):
                    items.append(("%s/No.%s/%s" % (kind, no, x["hg"]), x["svg"]))
    ...
    tmp = os.path.join(BASE, "scripts", "svg_check_daimon3.html")
    ...
    out = os.path.join(BASE, "scripts", "svg_findings_daimon3.json")
