  ...
  document.querySelectorAll('#screen-subject .subject-card').forEach(btn => {
    ...
    btn.onclick = () => {
      ...
      if (subj === 'sansu') {
        ...
        initTetris();
        showScreen('tetris');
      } else if (subj === 'jadepanic') {
        if (!hasGameTickets(JADEPANIC_COST)) return;
        initJadePanic();
        showScreen('jadepanic');
...
const BACKUP_KEYS = [
  ...
  // ⚠2026-08-28：8/24 に firestore.rules へは書いたのに、**公開を流し忘れていた**。
  //    そのあいだ この行はコメントのままで、記録は端末のみ＝キャッシュを消したら
  //    クリアした面が まるごと消えた（実被害あり）。コンソールの「最終公開日」が
  //    動いたのを確かめてから、この行を出している。
  //    ★次にキーを足すときも、必ず「rules を公開 → ここを足す」の順で。
  'okatazukeSave',
